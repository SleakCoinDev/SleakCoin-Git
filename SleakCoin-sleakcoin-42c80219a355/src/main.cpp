...
uint256 hashGenesisBlock("0x1ee5c793afa7172171cdaf94031569fd645789375afe98f6b3a63154ee554837");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SleakCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SleakCoin Signed Message:\n";
...
{
    // SleakCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SleakCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SleakCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // SleakCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1ee5c793afa7172171cdaf94031569fd645789375afe98f6b3a63154ee554837");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1408829398;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084544888;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Sleakcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SleakCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SleakCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SleakcoinMiner(CWallet *pwallet)
{
    printf("SleakCoinMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sleakcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SleakCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SleakCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SleakcoinMiner, pwallet));
}
